...
class ExamController extends Controller
{
	...
	public function showQuestionMaker($id){
		$exam = Exam::find($id);
		return view('student.pages.exams.showQuestionMaker', compact('id','exam'));
	}
	...
	public function showAnswerSheet($id){
		$exam = Exam::find($id);
		$hasAnswered = Result::where('user_id', Auth::user()->id)->where('exam_id', $id)->get();
		if( $hasAnswered->count() > 0 ){
			session()->flash('my_error', 'You have already answered');
			return redirect()->back();
		}
		return view('student.pages.exams.showAnswerSheet', compact('id','exam'));
	}
	...
	public function answerStore(Request $request){
		...
		return redirect()->route('student.courses.show', $exam->course->id);
	}
	...
	public function answerMark(Request $request){
		...
		return view('student.pages.exams.answerMark', compact('result', 'user_id', 'exam_id','exam'));
	}
