                    ...
                    <div class="card">
                        ...
                        <div class="body">
                            <div class="row text-left mb-4">
                              <div class="col-sm-12">
                                <h5><strong>ID: </strong> {{ $result->student->username }}</h5>
                              </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <input type="radio" name="a{{$loop->index+1}}" value="1" required

                                        @php
                                        	$index = $loop->index+1;

                                        	if( $result->{'a'.$index} == 1){ echo 'checked'; }
                                        @endphp

                                        > {{$question->option1}}
                                    </div>
                                    <div class="col-sm-12">
                                        <input type="radio" name="a{{$loop->index+1}}" value="2"

                                        @php
                                        	$index = $loop->index+1;

                                        	if( $result->{'a'.$index} == 2){ echo 'checked'; }
                                        @endphp                                           
                                    ...
                                    <div class="col-sm-12">
                                        <input type="radio" name="a{{$loop->index+1}}" value="3"


                                        @php
                                        	$index = $loop->index+1;

                                        	if( $result->{'a'.$index} == 3){ echo 'checked'; }
                                        @endphp
                                     
                                        > {{$question->option3}}
                                    </div>
                                    <div class="col-sm-12">
                                        <input type="radio" name="a{{$loop->index+1}}" value="4"

                                        @php
                                        	$index = $loop->index+1;

                                        	if( $result->{'a'.$index} == 4){ echo 'checked'; }
                                        @endphp                                           
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="a{{$loop->index+1}}" rows="5" required class="form-control"> 	
                            ...
                            <form action="{{ route('teacher.exams.markStore', $result->id) }}" method="POST" class="mt-5">
                                @csrf
                                <div class="form-group">
                                	<label for="">Total Marks</label>
                                	<input type="number" name="total_marks" value="{{$result->total_marks}}" step="0.1" required class="form-control">
                                </div>
                                <div class="form-group">
                                	<label for="">Result</label>
                                	<input type="text" name="result" value="{{$result->result}}" required class="form-control">
                                </div>
	                            <div class="form-group">
	                                <input type="submit" value="Submit" class="btn btn-primary">
	                                <a href="{{ route('teacher.courses.show', $exam->course->id ) }}" class="btn btn-danger">Go Back</a>
	                            </div>
                        	</form>
                        </div>
                    </div>
